# bdPrincipal.py  (PC3)
# Dos hilos:
#   - hiloEscritura : PULL en 6002, inserta eventos en mongo
#   - hiloConsulta  : REP  en 7001, atiende consultas síncronas del monitor
#                     y "ping" del healthcheck en PC2.
...
def menuConsultas(eleccion, bd):
    ...
    opciones = {
        ...
        "ping": lambda: {"status": "OK", "rol": "principal", "timestamp": str(datetime.now())},
    }
...
def hiloEscritura(bd, puerto):
    ...
    print(f"[BD Principal - Escritura] PULL en puerto {puerto}")
    ...
    while True:
        try:
            ...
            print(f"[BD Principal - Escritura] Guardado: {resultado.inserted_id}")
        except Exception as e:
            print(f"[Error bdPrincipal-Escritura] {type(e).__name__}: {e}")


def hiloConsulta(bd, puerto):
    """Hilo SÍNCRONO: atiende consultas REQ/REP y pings del healthcheck."""
    context = zmq.Context.instance()
    socket = context.socket(zmq.REP)
    socket.bind(f"tcp://0.0.0.0:{puerto}")
    print(f"[BD Principal - Consulta] REP en puerto {puerto}")
    ...
    while True:
        try:
            ...
            print(f"[Error bdPrincipal-Consulta] {type(e).__name__}: {e}")
            try:
...
def main():
    PUERTO_ESCRITURA = 6002
    PUERTO_CONSULTA  = 7001
    ...
    print("[BD Principal] Servicio iniciado (escritura asíncrona + consulta síncrona)")
